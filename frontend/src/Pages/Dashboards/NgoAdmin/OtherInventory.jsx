...
const OtherInventory = ({ onMarkUsed }) => {
  ...
  const fetchInventory = async () => {
    try {
      ...
      if (ngoData.inventory && ngoData.inventory.other) {
        setInventory(ngoData.inventory.other);
      }
    } catch (error) {
  ...
  const handleUse = async (item) => {
    try {
      const updatedInventory = await ngoService.deleteInventoryItem(ngo._id, "other", item._id);
  ...
  const handleSaveClick = async () => {
    try {
      const updatedInventory = await ngoService.updateInventoryItem(ngo._id, "other", editingId, editFormData);
      setInventory(updatedInventory);
      setEditingId(null);
      setEditFormData({});
    } catch (error) {
  ...
  const handleSaveNewItem = async () => {
    try {
      const updatedInventory = await ngoService.addInventoryItem(ngo._id, "other", newItemData);
      setInventory(updatedInventory);
      setIsAdding(false);
    } catch (error) {
