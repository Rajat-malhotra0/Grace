...
import { Shirt, Edit2, Save, X, Plus } from "lucide-react";
...
const ClothesInventory = ({ onMarkUsed }) => {
  ...
  const fetchInventory = async () => {
    try {
      ...
      if (ngoData.inventory && ngoData.inventory.clothes) {
        setInventory(ngoData.inventory.clothes);
      }
    } catch (error) {
  ...
  const handleUse = async (item) => {
    try {
      const updatedInventory = await ngoService.deleteInventoryItem(ngo._id, "clothes", item._id);
  ...
  const handleSaveClick = async () => {
    try {
      const updatedInventory = await ngoService.updateInventoryItem(ngo._id, "clothes", editingId, editFormData);
      setInventory(updatedInventory);
      setEditingId(null);
      setEditFormData({});
    } catch (error) {
  ...
  const handleSaveNewItem = async () => {
    try {
      const updatedInventory = await ngoService.addInventoryItem(ngo._id, "clothes", newItemData);
      setInventory(updatedInventory);
      setIsAdding(false);
    } catch (error) {
  ...
  return (
    <div className="inventory-section clothes-inventory">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem', borderBottom: '2px solid #eee', paddingBottom: '0.75rem' }}>
        <h2 className="section-title" style={{ margin: 0, border: 'none', padding: 0 }}>
            <Shirt size={24} style={{ marginRight: '0.5rem', verticalAlign: 'middle' }} />
            Clothes Inventory
        </h2>
        <button 
      ...
      <div className="inventory-card">
        ...
        {isAdding && (
            <div className="inventory-row" style={{ backgroundColor: '#f0f9ff' }}>
                ...
                <input type="text" placeholder="Size" value={newItemData.size} onChange={(e) => handleNewItemChange("size", e.target.value)} className="edit-input" />
                <input type="text" placeholder="Gender" value={newItemData.gender} onChange={(e) => handleNewItemChange("gender", e.target.value)} className="edit-input" />
          ...
          <div key={item._id} className="inventory-row">
            ...
            {editingId === item._id ? (
              <>
                ...
                <input type="text" value={editFormData.size} onChange={(e) => handleFormChange("size", e.target.value)} className="edit-input" />
                <input type="text" value={editFormData.gender} onChange={(e) => handleFormChange("gender", e.target.value)} className="edit-input" />
