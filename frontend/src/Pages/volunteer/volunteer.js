...
function Volunteer() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const volunteerRes = await axios.get("api/users?role=volunteer");
            const tasksRes = await axios.get("api/tasks");
            const surveysRes = await axios.get("api/skill-surveys");
            const processed = processData(
                volunteerRes.data.result || [],
    ...
    const processData = (volunteers, surveys, tasks) => {
        const timeFrames = {
            week: 7,
            month: 30,
            quarter: 90,
        ...
        const activeVolunteers = volunteers.filter((v) => {
            ...
            const diff = isNaN(updatedAtDate.getTime())
                ? Infinity
                : now - updatedAtDate.getTime();
            return diff < days;
        }).length;

        const newVolunteers = volunteers.filter((v) => {
            const createdAtDate = new Date(v.createdAt);
            const diff = isNaN(createdAtDate.getTime())
                ? Infinity
                : now - createdAtDate.getTime();
            return diff < days;
        }).length;
        ...
        const maxTotalHours = Math.max(
            ...Object.values(roleStats).map((r) => r.hours),
            1
        ...
        tasks.forEach((task) => {
            if (task.assignedTo) {
                volunteerTasks[task.assignedTo] =
                    (volunteerTasks[task.assignedTo] || 0) + 1;
            }
        });
        ...
        surveys.forEach((s) => {
            (s.skills || []).forEach(
                (skill) => (skills[skill] = (skills[skill] || 0) + 1)
            );
            (s.interests || []).forEach(
                (interest) =>
        ...
        const retentionRate =
            totalVolunteers > 0
                ? (
                      (topVolunteers.filter((v) => v.isRepeat).length /
                          totalVolunteers) *
                      100
                  ).toFixed(1)
                : 0;
                ...
                <div className="availability-stats">
                    <div className="availability-overview">
                        ...
                        <p>
                            ...
                            <strong>{data.availability.totalSurveys}</strong>
                        </p>
                        <p>
                            Average Hours Available:{" "}
                            <strong>
                                {(data.availability.avgHours ?? 0).toFixed(1)}{" "}
                                hours/week
                            </strong>
                        </p>
                    </div>
                    ...
                    <div className="availability-distribution">
                        ...
                        {Object.entries(data.availability.hourRanges || {}).map(
                            ([range, count]) => (
