...
function Service() {
    ...
    const fetchVolunteerInsights = async () => {
        ...
        try {
            ...
            const processedData = processVolunteerData(
                users.result || [],
                skillSurveys.result || [],
                tasks.result || []
            );
            ...
            // Error fetching volunteer insights
        }
        setLoading(false);
    };

    const processVolunteerData = (users, surveys, tasks) => {
        const volunteers = users.filter((user) =>
        ...
        tasks.forEach((task) => {
            if (task.assignedTo && task.actualMinutes > 0) {
                ...
                if (volunteer) {
                    ...
                    roleEngagement[category].totalHours +=
        ...
        const mostEngagedRoles = Object.values(roleEngagement)
            .map((role) => ({
                ...
                avgHoursPerVolunteer:
                    role.totalHours / role.volunteers.size || 0,
                engagementScore:
                    (role.totalHours * role.completedTasks) /
                    Math.max(role.volunteers.size, 1),
            }))
        ...
        tasks.forEach((task) => {
            if (task.assignedTo) {
                volunteerTaskCounts[task.assignedTo] =
                    (volunteerTaskCounts[task.assignedTo] || 0) + 1;
            }
        });
            ...
            .map(([volunteerId, taskCount]) => {
                ...
                return volunteer
                    ? {
                          ...
                          totalHours:
                              tasks
                                  .filter((t) => t.assignedTo === volunteerId)
                                  .reduce(
        ...
        const availabilityStats = {
            ...
            avgHours:
                surveys.reduce(
                    (sum, s) => sum + (s.availabilityHours || 0),
                    0
        ...
        surveys.forEach((survey) => {
            ...
            const range =
                hours === 0
                    ...
                    ? "11-20"
                    : "20+";
            availabilityStats.hourDistribution[range] =
                (availabilityStats.hourDistribution[range] || 0) + 1;
        });
        ...
        surveys.forEach((survey) => {
            (survey.skills || []).forEach((skill) => {
                quizMatchStats.skillCategories[skill] =
                    (quizMatchStats.skillCategories[skill] || 0) + 1;
            });
            (survey.interests || []).forEach((interest) => {
                quizMatchStats.interestCategories[interest] =
                    (quizMatchStats.interestCategories[interest] || 0) + 1;
            });
        });
        ...
        return {
            ...
            retentionRate: (
                (repeatVolunteers.filter((v) => v.isRepeat).length /
                    Math.max(volunteers.length, 1)) *
                100
            ).toFixed(1),
            mostEngagedRoles,
            volunteerOrigins,
            ...
            <div className="metrics-overview">
                <div className="metric-card">
                    ...
                    <span className="metric-value">
                        {volunteersData.totalVolunteers}
                    </span>
                </div>
                <div className="metric-card">
                    <h3>Active Volunteers</h3>
                    <span className="metric-value">
                        {volunteersData.activeVolunteers}
                    </span>
                </div>
                <div className="metric-card">
                    <h3>New Volunteers</h3>
                    <span className="metric-value">
                        {volunteersData.newVolunteers}
                    </span>
                </div>
                <div className="metric-card">
                    <h3>Retention Rate</h3>
                    <span className="metric-value">
                        {volunteersData.retentionRate}%
                    </span>
                </div>
            </div>
                    ...
                    {volunteersData.mostEngagedRoles.map((role, index) => (
                        <div key={index} className="role-card">
                            ...
                            <div className="role-stats">
                                <p>
                                    <strong>{role.volunteerCount}</strong>{" "}
                                    volunteers
                                </p>
                                <p>
                                    <strong>
                                        {role.totalHours.toFixed(1)}
                                    </strong>{" "}
                                    total hours
                                </p>
                                <p>
                                    <strong>{role.completedTasks}</strong> tasks
                                    completed
                                </p>
                                <p>
                                    <strong>
                                        {role.avgHoursPerVolunteer.toFixed(1)}
                                    </strong>{" "}
                                    avg hours/volunteer
                                </p>
                            </div>
            ...
            <div className="insight-section">
                ...
                <div className="origins-container">
                    <div className="origins-chart">
                        {volunteersData.volunteerOrigins.map(
                            (origin, index) => (
                ...
                <div className="availability-stats">
                    <div className="availability-overview">
                        ...
                        <p>
                            ...
                            <strong>
                                {volunteersData.availabilityStats.totalSurveys}
                            </strong>
                        </p>
                        <p>
                            Average Hours Available:{" "}
                            <strong>
                                {volunteersData.availabilityStats.avgHours.toFixed(
                                    1
                                )}{" "}
                                hours/week
                            </strong>
                    ...
                    <div className="availability-distribution">
                        ...
                        {Object.entries(
                            volunteersData.availabilityStats.hourDistribution
                        ).map(([range, count]) => (
                    ...
                    <div className="quiz-overview">
                        <div className="quiz-metric">
                            ...
                            <span className="large-number">
                                {volunteersData.quizMatchStats.matchAccuracy}%
                            </span>
                        </div>
                        <div className="quiz-metric">
                            <h4>Total Matches</h4>
                            <span className="large-number">
                                {volunteersData.quizMatchStats.totalMatches}
                            </span>
                        </div>
                    </div>
