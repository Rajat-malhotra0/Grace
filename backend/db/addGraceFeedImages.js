//Use this to add images to the db 
...
async function addGraceFeedImages() {
    try {
        ...
        console.log(`Successfully added ${savedPosts.length} Grace feed images dynamically!`);
        ...
        savedPosts.slice(0, 3).forEach((post, index) => {
            console.log(`Post ${index + 1}: ${post.content} - "${post.caption}"`);
        });
